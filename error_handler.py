...
class ErrorMetrics:
    ...
    def _send_alert(self, error_rate: float) -> None:
        ...
        # Логируем детали
        logger.error(f"  Ошибки по типам: {json.dumps(self.errors_by_type, ensure_ascii=False)}")
...
def log_error_json(
    ...
    logger.error(f"  {brand}/{article}: {error.__class__.__name__}: {str(error)[:120]}...")
...
def handle_parse_errors(func: Callable) -> Callable:
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            result.products.append(Product(price_text=f"ошибка: {str(e)[:50]}"))
            return result
